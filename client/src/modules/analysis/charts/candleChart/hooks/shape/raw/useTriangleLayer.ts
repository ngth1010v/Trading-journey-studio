...
export default function useTriangleLayer(): TriangleLayer {
  ...
  const add = (triangle: Triangle): void => {
    ...
    if (currentIdx >= CONFIG.LINES_LAYER.MAX_LINES) {
      throwAppError("LIMIT_EXCEEDED", "Triangle layer capacity limit has been reached.");
    }
    ...
    if (
      ...
      throwAppError("MISSING_ARGUMENT", "Triangle object is missing required rendering positional vertex attributes.");
    }
